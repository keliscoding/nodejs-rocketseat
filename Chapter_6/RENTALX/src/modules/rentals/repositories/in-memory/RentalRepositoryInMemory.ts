...
import { ICreateRentalDTO, IRentalRepository } from '@modules/rentals/repositories/IRentalRepository';
...
class RentalRepositoryInMemory implements IRentalRepository{
    ...
    async findByUserId(id: string): Promise<Rental[]> {
        const rentals = this.rentals.filter( rentals => rentals.user_id = id);
        return rentals;
    }
}
